...
export class CattwoComponent extends PagebaseService implements OnInit {
  selectAll:any;
  ...
  constructor(private activatedRoute: ActivatedRoute,private router: Router,private _ajax: AjaxService, private _elem: ElementService,private _util: UtilsService,private _expend: ExtendsService) {
    super();
  ...
  let parm=  {"paged":true,"page":1,"limit":10};
    ...
    this._ajax.CorePost(`fcmanager/catAdvanceTicket/list`,parameters, 2).subscribe(responses => {
      debugger;
      if (responses.code == "0") {
 ...
 EditSave(){
  ...
  if (newObj) {
  let parameters={
   returnTripAllowOpen:newObj.type,
    advanceTicketTimeunit:newObj.advanceTicketTimeunit,
   advanceTicketTime:newObj.advanceTicketTime,
   id:newObj.id
  };
...
btnGoBack(): void {
  ...
  btnDel(item): void {
    this._elem.confirm("确定要删除吗？", () => {
      this._ajax.CoreGet(`fcmanager/catAdvanceTicket/del/${item.id}`, 2).subscribe(responses => {
        debugger;
        if (responses.code == "0") {
          this._elem.tip(`删除成功`);
          this.btnSearch(null);
